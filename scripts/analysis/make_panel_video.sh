#!/bin/bash
set -e
STRIDE=${1:-1}  # must match the stride passed to make_field_heatmaps.py
...
for sub in oxygen glucose Gal8_ext lactate ECM TGF_beta IFN_gamma; do
    n=0
...
while [ $n -lt $N_FRAMES ]; do
    idx=$(printf "%04d" $n)
    t=$((n * 60 * STRIDE))
    convert \( oxygen/panel_${idx}.png glucose/panel_${idx}.png Gal8_ext/panel_${idx}.png lactate/panel_${idx}.png +append \) \
            \( ECM/panel_${idx}.png TGF_beta/panel_${idx}.png IFN_gamma/panel_${idx}.png +append \) \
            -background white -append -gravity North -background white -splice 0x40 \
            -pointsize 22 -annotate +0+10 "t = ${t} min ($(awk "BEGIN{printf \"%.1f\", ${t}/1440}") days)" \
